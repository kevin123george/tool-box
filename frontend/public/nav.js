...
(function () {
    ...
    const ICONS = {
        ...
        guide: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
        ...
        docker: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="w-5 h-5 shrink-0">
            <rect x="2" y="9" width="3" height="3" rx="0.5"/><rect x="6" y="9" width="3" height="3" rx="0.5"/><rect x="10" y="9" width="3" height="3" rx="0.5"/><rect x="6" y="5" width="3" height="3" rx="0.5"/><rect x="10" y="5" width="3" height="3" rx="0.5"/>
            <path stroke-linecap="round" stroke-linejoin="round" d="M20.5 10c-.5-1-1.8-1.5-3-1.2-.3-1.2-1.2-2.3-2.5-2.8v1c.8.4 1.4 1.1 1.5 2H14v1.5h7c0 2-1.5 3.5-3.5 3.5H4.5C3 14.5 2 13.3 2 12"/>
        </svg>`,
        ...
        downloads: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3"/>
        </svg>`,
        notes: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 shrink-0">
            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"/>
        </svg>`,
        settings: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z"/>
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
        </svg>`,
    ...
    const MENU_GROUPS = [
        ...
        },
        {
            ...
            adminOnly: true,
            items: [
                { key: 'admin',     label: 'Users',     href: '/admin.html',     icon: 'users'     },
                { key: 'system',    label: 'System',    href: '/system.html',    icon: 'system'    },
                { key: 'docker',    label: 'Docker',    href: '/docker.html',    icon: 'docker'    },
                { key: 'downloads', label: 'Downloads', href: '/downloads.html', icon: 'downloads' }
            ]
        }
    ];
                    ...
                    <div class="navbar-end gap-2">
                        ...
                        <div class="dropdown dropdown-end">
                            ...
                            <ul tabindex="0" class="dropdown-content z-[200] menu bg-base-100 rounded-xl shadow-lg border border-base-200 p-1.5 w-44 mt-2">
                                ...
                                <li><a onclick="openServerSettings()" class="flex items-center gap-2 rounded-lg text-sm">
                                    ${ICONS.settings} Server URL
                                </a></li>
...
function openServerSettings() {
    ...
    const current = localStorage.getItem('apiBaseUrl') || window.__API_BASE || '';
    const modal = document.createElement('dialog');
    modal.id = 'serverSettingsModal';
    modal.className = 'modal modal-open';
    modal.innerHTML = `
        <div class="modal-box max-w-sm">
            <h3 class="font-bold text-lg mb-1">Server URL</h3>
            <p class="text-sm opacity-50 mb-4">Set your Tailscale or local backend address. Takes effect immediately.</p>
            <label class="label text-xs opacity-50 mb-1">API Base URL</label>
            <input id="serverUrlInput" type="url" class="input input-bordered w-full font-mono text-sm"
                   placeholder="http://your-server:9099" value="${current}" />
            <div class="modal-action mt-5">
                <button onclick="document.getElementById('serverSettingsModal').remove()" class="btn btn-ghost btn-sm">Cancel</button>
                <button onclick="saveServerUrl()" class="btn btn-primary btn-sm">Save & Reload</button>
            </div>
        </div>
        <form method="dialog" class="modal-backdrop"><button>close</button></form>
    `;
    document.body.appendChild(modal);
    setTimeout(() => modal.querySelector('#serverUrlInput')?.focus(), 50);
}
