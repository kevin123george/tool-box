...
#                           Exposed on NodePort 30090 (HTTP, no TLS)
set -e
...
# ── 2. Patch deployment to run in HTTP mode ───────────────────────────────────
echo ""
echo "==> [2/4] Switching dashboard to HTTP mode..."
kubectl patch deployment kubernetes-dashboard \
  ...
  -p='[
    ...
    {"op":"add","path":"/spec/template/spec/containers/0/args/-","value":"--insecure-port=9090"},
    {"op":"add","path":"/spec/template/spec/containers/0/args/-","value":"--insecure-bind-address=0.0.0.0"}
  ]'

kubectl patch svc kubernetes-dashboard \
  -n kubernetes-dashboard \
  -p '{"spec":{"type":"NodePort","ports":[{"port":9090,"targetPort":9090,"nodePort":30090,"protocol":"TCP"}]}}'
ok "HTTP mode on NodePort 30090 configured"
...
echo "  (or)   http://<tailscale-ip>:30090"
echo ""
